 ...
 *             secret:         Azure secret. Must specify this or secretId.
 *             secretId:       ID of secret in rest storage. Must specify this or secret.
...
AzureAutoscaleProvider.prototype.init = function init(providerOptions) {
    ...
    credentialsPromise
        .then((data) => {
            ...
            if (credentialsJson.secretId) {
                return getSecret(credentialsJson);
            }
            return q(credentialsJson.secret);
        })
        .then((data) => {
            credentialsJson.secret = data;
...
function getSecret(credentials) {
    ...
    hash.update('+');
    hash.update(credentials.tenantId);
    return localCryptoUtil.decryptDataFromRestStorage(credentials.secretId, hash.digest('hex'))
        .then((data) => {
            return data.servicePrincipalSecret;
        })
        .catch((err) => {
