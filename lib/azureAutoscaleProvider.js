...
AzureAutoscaleProvider.prototype.getStoredUcs = function() {
    ...
    this.storageClient.listBlobsSegmented(BACKUP_CONTAINER, null, null, function(err, result) {
        ...
        if (newest.container) {
            ...
            blobStream = this.storageClient.createReadStream(BACKUP_CONTAINER, newest.name, function(err) {
                if (err) {
                    this.logger.warn('blob createReadStream failed:', err);
                    deferred.reject(err);
                    return;
                }
            }.bind(this));
            ...
            blobStream.on('finish', function() {
                this.logger.silly('finished reading blob');
                deferred.resolve(blobStream);
            }.bind(this));

            blobStream.on('error', function(err) {
                this.logger.warn('error reading blob stream', err);
                deferred.reject(err);
            }.bind(this));
        }
        else {
