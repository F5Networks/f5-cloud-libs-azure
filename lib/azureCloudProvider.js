...
AzureCloudProvider.prototype.init = function init(providerOptions) {
    ...
    credentialsPromise
        .then((data) => {
            if (providerOptions.azCredentialsUrl && providerOptions.azCredentialsEncrypted) {
    ...
    this.bigIp.init(
        ...
        .then(() => {
            if (this.instancesToRevoke.length > 0) {
                logger.debug(
                    'Revoking licenses of non-masters that are not known to Azure',
                    this.instancesToRevoke
                );
 ...
 *                    elected master.
 */
AzureCloudProvider.prototype.electMaster = function electMaster(instances) {
 ...
 *                    }
 */
AzureCloudProvider.prototype.getMasterCredentials = function getMasterCredentials() {
    return q({
 ...
 */
AzureCloudProvider.prototype.isValidMaster = function isValidMaster(instanceId, instances) {
 ...
 */
AzureCloudProvider.prototype.masterElected = function masterElected(instanceId) {
 ...
 * Called when a master has been elected.
 *
 * Update VirtualMachineScaleSet Tags, adding/updating the Deployment master tag.
 *
 * @param {String} masterId - The instance ID of the elected master.
 * @param {Object} instances - Dictionary of instances as returned from getInstances.
 *
 * @returns {Promise} A promise which will be resolved when processing is complete.
 */
AzureCloudProvider.prototype.tagMasterInstance = function tagMasterInstance(masterIid, instances) {
    if (!instances[masterIid]) {
        return q.reject(new Error('Master Instance provided not in instances dictionary'));
    }
    return getTagsFromScaleSet(this.computeClient, this.resourceGroup, this.scaleSet)
        .then((tags) => {
            const scaleSetTags = {};
            scaleSetTags.tags = tags;
            scaleSetTags.tags[`${this.resourceGroup}-master`] = instances[masterIid].privateIp;
...
function getScaleSetVmsInstanceView(computeClient, resourceGroup, scaleSetName, instanceId) {
    assert.ok(computeClient, 'getScaleSetVmsInstanceView: no compute client');
    assert.ok(resourceGroup, 'getScaleSetVmsInstanceView: no resource group');
    assert.ok(scaleSetName, 'getScaleSetVmsInstanceView: no scaleSetName group');
    assert.ok(instanceId, 'getScaleSetVmsInstanceView: no instanceId group');
...
function getScaleSetNetworkPrimaryInterfaces(
    ...
    assert.ok(computeClient, 'getScaleSetNetworkPrimaryInterfaces: no network client');
...
function getJsonObject(storageClient, container, name) {
    assert.ok(storageClient, 'getJsonObject: no storage client');
    assert.ok(container, 'getJsonObject: no container');
    assert.ok(name, 'getJsonObject: no name');
