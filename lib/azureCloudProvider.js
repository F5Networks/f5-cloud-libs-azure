    ...
    getScaleSetVms(this.computeClient, this.resourceGroup, this.scaleSet, { expand: 'instanceView' })
        ...
        .then((registeredInstances) => {
            ...
            if (registeredInstanceIds.length > 0) {
                isMaster = registeredInstanceIds.filter((id) => {
                    return currInstanceId === id && registeredInstances[currInstanceId].isMaster;
                }).length === 1;
            }
            logger.silly(`isMaster: ${isMaster}`);
...
AzureCloudProvider.prototype.electMaster = function electMaster(instances) {
    ...
    logger.silly('electMaster: checking if lowest ip has running config');
    ...
    if (!isLowestInstanceWithRunningConfig && instancesWithRunningConfig.length > 0) {
        logger.silly('electMaster: elected master does not have running config');
        logger.silly('electMaster: taking lowest with running config');
        instancesWithRunningConfig.sort((instance01, instance02) => {
            return instance01.mgmtIp - instance02.mgmtIp;
        });
        logger.silly(`electMaster: instance after sort: ${instancesWithRunningConfig}`);
